...
```json
{
  ...
  "contact_channel": { "type": "email|discord|telegram|whatsapp|imessage|signal|phone|instagram|twitter|linkedin|matrix|line", "value": "<val>" },
...
**Shared `thread_id`:** `match --start` mints via `POST /negotiations` (signed body `{"pubkey":"<hex>","peer_pubkey":"<hex>"}`). The registry returns `{ "thread_id": "<uuid>", "reused": <bool> }`. If an open negotiation for that pair already exists (within 7 days), `reused: true` and the **same** `thread_id` is returned — the client must create **responder** state (`weInitiated: false`), not a second initiator. Prefer **only one** `--start` per pair; the peer should ingest Nostr first. Both parties must be registered before minting.
...
**Actions:**
```bash
matchclaw handoff --status                                    # list active handoffs
...
`HandoffState.status` progression: `pending_consent → round_1 → complete` (or `expired` after **60h**). Round 1 debrief must wrap up before Round 2 exchange.

- **Round 1** — Surface the match, run a private debrief, record user consent
- **Round 2** — Frame the moment, run `--exchange` to surface peer contact, withdraw

`pending_notification.json`:
```json
{
  "introId": "<uuid>", "remoteKey": "<hex>",
  "narrative": {}, "lockedAt": "<iso8601>"
}
```
...
{
  "introId": "<uuid>", "remoteKey": "<hex>",
  "stage": 1, "status": "pending_consent",
  "narrative": {}, "created_at": "<iso8601>",
  "agreedAt": "<iso8601>",
  "remoteContact": { "type": "<string>", "value": "<string>" }
}
```
